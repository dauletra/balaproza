...
# Абсолютный адрес портала. Нужен ссылкам, которые уходят **наружу**:
# сообщение бота открывается не на нашей странице, и относительный путь
# там не адрес, а строка. Узнать домен без запроса Django не умеет —
# `django.contrib.sites` не подключён и ради одной строки не нужен.
#
# Явная переменная сильнее вывода: за прокси первый разрешённый хост
# бывает внутренним именем, а не тем, что читатель видит в браузере.
...
# роли не существует, любой зарегистрированный сразу и пишет.
...
# Оба порога — про буферизацию/тело
