...
Значения вписаны литералами и не читаются из `core/stub_data.py`. Данные
миграции заморожены на момент её написания: импорт живого модуля означал
бы, что уже применённая миграция меняет смысл при следующей правке кода,
а после удаления стаба (docs/19 §19.4) вовсе перестанет импортироваться.
"""
