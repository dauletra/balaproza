...
def _copy_tag_links(apps, schema_editor):
    ...
    Настоящий момент, когда автор поставил тег, нигде не хранился (см.
    docstring `Tag`) — `created_at` приближён датой создания работы: то же
    допущение, на котором стоят остальные денормализованные числа демо-
    корпуса. Идёт ДО смены источника `Story.tags` ниже — иначе читал бы
    уже несуществующую старую m2m-таблицу.
    """
