...
def auth_state(request):
    """Гость ↔ вошедший для шаблонов (шапка, нижнее меню, рейл).

    Отвечает `request.user`, а не session-флаг: вход настоящий. `signed_in`
    рядом с `user.is_authenticated` — тот же ответ под привычным шаблонам
    именем, а не второй источник.
