...
Фаза выводится из трёх дат, но её имена и подписи — словарь
предметной области: их показывают бейдж, рейл и уведомление, и собирать
слово в шаблоне запрещено ровно так же, как у статусов работы.

`SUBMISSION_NOTES` — заметки, а не запреты: форма ничего не
...
# Фазы конкурса. Порядок — хронологический.
...
# Путь заявки: подана — рассмотрена — решение. Победа сюда не
# входит: она приходит присуждением награды, а не сменой статуса,
# потому что «первое место» — акт жюри, а не состояние документа.
SUBMISSION_STATUSES = ("reviewing", "accepted", "rejected")

# Ответ формы подачи на вопрос об AI-помощи. Пишется один раз при
# создании заявки, дальше поле read-only — автор его не редактирует.
AI_DECLARATIONS = ("no", "partial", "yes")

# Коротко — для списка заявок у жюри. Автору в форме подачи вопрос задан
# полными фразами (`contest_submit.html`): там это ответ, здесь — метка.
...
# Подписи результата — те же слова, что в «Менің өтінімдерім»,
...
# Статус заявки называется теми же словами, что результат у автора в
# «Менің өтінімдерім»: словарь один, «Жеңімпаз» в нём — сверх статусов.
SUBMISSION_STATUS_LABELS = {s: CONTEST_RESULT_LABELS[s]
                            for s in SUBMISSION_STATUSES}

# Что из результата видит посторонний. Всё остальное публично
# выглядит просто участием.
PUBLIC_CONTEST_RESULTS = ("winner", "accepted")


# Что стоит знать автору о работе перед подачей. Это **заметки,
