...
def status_after_moderation(outcome: str, story_format: str) -> str:
    ...
    Здесь, а не в админке: это часть модели публикации, а админка — всего
    лишь сегодняшний инструмент модерации (DEC-23).

    Одобрение **не ставит литерал `Published`** — статус зависит от формата
    (BR-10a, DEC-37): у сериала `Published` невалиден. «Дописан» — слово
    автора, а не модератора, и `Completed` ставится отдельно.

    Оба отрицательных исхода возвращают работу в черновики; разница между
    ними живёт в уведомлении (BR-72b), а не в статусе.
    """
    if outcome == "approved":
