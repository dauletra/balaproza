...
def from_annotation(instance, annotation: str, compute):
    ...
    Контракт между выдачей и моделью был выражен восемью
    `getattr(self, '<имя>', None)`, разбросанными по `models.py`: забыл
    аннотацию — страница работает и делает запрос на каждую строку, а
    заметить это можно только бюджетом запросов, то есть на тех страницах,
    которые в него внесены.

    Теперь у контракта одно имя и одна дверь, а промах называет себя вслух:
    в `DEBUG` он пишется в лог вместе с именем аннотации. В проде поведение
    прежнее — правильный ответ ценой запроса; терять страницу из-за
    забытого `annotate` было бы хуже самой ошибки.

    Строка в логе не объявляет ошибку: у одиночного объекта запрос — это
    один запрос, и так и задумано (страница произведения спрашивает число
    работ автора). Ошибку показывает **повтор**: двадцать одинаковых строк
    на одну страницу и есть N+1, который иначе виден только бюджетом.
    """
...
def viewer_choice(instance, mark: str) -> str:
    """Что выбрал этот читатель: реакция на главе, голос в опросе.

    Не аннотация выдачи, а метка, которую ставит `queries/story.py`, и
    промах здесь дороже: пустая строка значит «не голосовал», и забытая
    метка не удорожает страницу, а **врёт** — читатель видит свой голос
    несделанным, а второй клик молча заводит ещё одну строку.

    Поэтому метка ставится всегда, гостю тоже (пустой), а её отсутствие —
    сломанный контракт, а не ответ «голоса нет».
    """
