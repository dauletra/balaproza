...
def export_texts(request):
    ...
    тексты не просят, — а против скрипта, дёргающего адрес в цикле:
    каждый проход собирал бы романы заново. Отказ говорит словами и
    возвращает на страницу профиля, а не отвечает кодом: сюда приходят в
    худший день, и пустой ответ браузера читался бы как «пропало и это».
    """
    if data.export_too_often(request.user):
        messages.info(request, 'Мәтіндерің жаңа ғана жүктелді — сәл кідір.')
        return redirect('core:profile_me_edit')
