...
# ───────────────────────── Демо-состояния ────────────────────────────────
# `?state=loading|error` превращает страницу в скелетон или ошибку.
# Пока данные приходят синхронно, показать эти два состояния больше
# нечем — на проде их даст async-загрузка через htmx.
#
# Только при `DEBUG`, и это не формальность: переключатель был открыт
# всем, то есть любой посетитель мог открыть главную с `?state=error` и
# увидеть «жүктеу мүмкін болмады» там, где всё работает.
_PAGE_STATES = ('content', 'loading', 'error')


def _found_or_404(obj, what: str):
    """Публичный объект или 404 — как у профиля с первого дня.
