...
class Contest:
    ...
    def eligibility_line(self) -> str:
        ...
        Собирать эту строку в шаблоне запрещено — то же правило, что
        у `CONTEST_PHASE_LABELS`: её показывают чек-лист подачи, секция
        условий и чекбокс подтверждения, и три рукописные копии разошлись
        бы у первого же конкурса с другой вилкой.
        """
    ...
    def other_editions(self) -> list:
        """Другие выпуски того же конкурса, свежие сверху (BR-47).

        Завершённый конкурс без этого — тупик: страница «Жас алдым — 2023»
        заканчивалась составом жюри, и читатель, которого привёл поиск,
        уходил ни с чем, хотя приём в выпуск этого года шёл прямо сейчас.
        Связь по слагу семейства, а не по совпадению имён: у выпусков
        имена расходятся («Жас алдым — 2023» против «Жас алдым»).
        """
        if not self.series:
            return []
        return sorted((c for c in CONTESTS
                       if c.series == self.series and c.slug != self.slug),
                      key=lambda c: c.results_on, reverse=True)
...
def common_rules(contest: "Contest") -> list:
    ...
    Раньше каждый конкурс переписывал их себе в `conditions` руками, и
    копия успела разойтись сразу тремя способами. Неполно: AI-декларация
    обязательна для всех (DEC-21), а названа была у одного конкурса из
    пяти — на остальных страницах выходило, что там её не спросят.
    Литералом: «5 000-15 000 таңба» стояло текстом при том, что пороги
    хранятся в `min_chars`/`max_chars` и у каждого конкурса свои
    (FR-CONT-07). И с кодами ТЗ: «(BR-23)», «(DEC-21)» читал подросток.

    `per_work` — проверяется ли правило у конкретной работы. «Бір автор —
    бір өтінім» относится к автору, а не к тексту, и в чек-лист подачи
    не идёт: его и так держит сама форма (BR-23).
    ...
    return [
        ...
        # Единственное общее правило, которое конкурс вправе сузить, —
        # и оговорка стоит прямо в нём. Пока общие правила показывались
        # отдельным блоком, «Тек қазақ тілінде» у «Алтын қалам» читалось
        # как сужение; в одном списке рядом с безусловным «қазақша немесе
        # орысша» те же две строки читаются как противоречие.
...
def submission_checklist(story: "Story", contest: "Contest") -> list:
    ...
    Общая часть приходит из `common_rules` — второй рукописной копии тех
    же правил в проекте быть не должно. Возрастной пункт добавляется
    только когда конкурс ставит вилку (BR-48); раньше «Жас 14-18» стоял
    всегда, всегда «пройден», и обещал сверку с полем профиля, которого
    у `Author` нет вовсе.

    «Объём» — единственная авто-проверка; остальное требует декларации автора.
    """
    ...
    # Состояние каждого правила: что считается автоматически, что требует
    # ответа автора. Сами правила и их подписи — в `common_rules`.
    state = {
    ...
    # Возраст — требование конкурса, а не платформы (BR-48). Конкурс без
    # вилки не добавляет пункта вовсе: вечно «пройденная» строка ничего
    # не сообщает, а «Жас 14-18» у конкурса с другой вилкой ещё и врала.
