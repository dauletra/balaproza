...
# Лимит аннотации. У поля модели его нет — это `TextField`, — и
# счётчик в шаблоне до этого был единственным местом, где число называлось.
ANNOTATION_MAX = 500

# Границы опроса. Верхняя — не «пока столько влезло»: список
...
class NewStoryForm(forms.ModelForm):
    """Создание произведения — три поля. Статуса здесь нет:
    новая работа всегда черновик, и форма сообщает это строкой."""
    ...
    def clean(self):
        """Потолок пустых черновиков: ничем
...
class StorySettingsForm(forms.ModelForm):
    """Баптаулар произведения.

    Три правила, которые нельзя обойти прямым POST'ом: статус выбирает
    только публичный сериал, одночастная форма не даётся
    работе с несколькими главами, обложка обязана быть растром.
    ...
    # слоем данных (pending → accepted), жанры — слагами.
    tags = forms.CharField(required=False)
    # Явное снятие обложки — третье состояние рядом с «новый файл»
    ...
    def clean_status(self):
        """Радио статуса рендерится только публичному сериалу. POST
...
class ChapterForm(forms.ModelForm):
    """Редактор главы вместе с необязательным опросом.
    ...
    def clean(self):
        """Вопрос и варианты существуют только вместе.
...
class ChapterAutosaveForm(forms.ModelForm):
    """Автосохранение главы — те же два поля, но без обязательных.
...
class ProfileForm(forms.ModelForm):
    """Редактирование своего профиля. `birth_date` и `gender` —
    самодекларация; пустой `avatar` значит «не меняем»."""

    # Явное снятие аватара — см. `remove_cover` у `StorySettingsForm`.
    remove_avatar = forms.BooleanField(required=False)

    # Вне Meta.fields: `username` у модели `blank=False`, и
...
class OnboardingForm(forms.ModelForm):
    """Онбординг после первого Telegram-входа.
...
class SubmissionForm(forms.Form):
    ...
    **Форма ничего не отклоняет по содержанию работы**: рядом с
    ...
    ли AI-декларация, подтверждены ли возраст и правила.
    ...
    def __init__(self, *args, candidates=None, contest=None, **kwargs):
        ...
        # у конкурса без ценза чекбокса нет вовсе.
...
class CommentForm(forms.Form):
    """Комментарий или ответ на него. Уровень
