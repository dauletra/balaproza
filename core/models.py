...
Правило файла — **производное не хранится** (DEC-40, DEC-45): число работ
автора, счётчик жанра, популярность тега считаются запросом. Колонка
заводится, только если значение нельзя вывести (акт человека) или если
агрегат по логу слишком дорог, — и тогда рядом стоит его пересчёт.

Подписи здесь не собираются: это работа `templatetags/balaproza` и
`domain/`. Выдача — работа `managers.py`.
"""
...
# открывается в origin сайта, а SVG — документ со скриптами. По расширению,
# а не по содержимому: `ImageField` потребовал бы Pillow ради одного поля.
...
def story_cover_path(instance, filename):
    """`covers/<slug>.<ext>`: загруженное имя («IMG_20240817 (1).PNG»)
    попало бы в публичный URL навсегда, а слаг уникален и читается."""
...
class User(AbstractUser):
    ...
    зарегистрировавшийся сразу и читает, и пишет.

    `first_name` / `last_name` убраны: казахское имя не делится на две
    западные графы, а имён у нас и так два — настоящее (`name`, для
    модератора и жюри) и публичное (`pen_name`). Пустые поля
    `AbstractUser` стали бы третьим и четвёртым местом хранения имени.
    """
    ...
    # причине, что `Story.likes`: её читают `ORDER BY` ленты «Жаңа авторлар»
    # и `WHERE` оси каталога. Пересчитывается по строкам `Follow`, а не
    # сдвигается на единицу, — так она сама себя исправляет.
    ...
    class Meta:
        ...
        indexes = [
            # Поиск ищет автора всеми тремя именами. Это `ILIKE`
            # с подстрокой, и B-tree его не берёт — совпадение начинается
            # не с начала строки. Триграммный берёт и сам складывает регистр.
    ...
    def get_short_name(self):
        ...
        Единственное место, где к пользователю обращаются, а не
        рассказывают о нём третьему, — потому имя, а не `public_name`, и
        только первое слово: полное имя с обращением на «сен» (docs/ui.md)
        звучит как вызов к доске.
        """
    ...
    @cached_property
    def works(self) -> int:
        """Сколько работ автора видит читатель (DEC-40).

        Черновики не в счёт: иначе число выдаёт читателю, что у автора
        есть неопубликованное. Списки авторов подставляют аннотацию
        `works_count`; одиночному автору запрос законен, но ровно один —
        страница произведения спрашивает число дважды.
        """
    ...
    # Страница профиля спрашивает работы автора из восьми мест. Снимок живёт
    # на самом объекте: `cached_property` кэширует в `self.__dict__`, то есть
    # на время жизни экземпляра, а он живёт один запрос (`request.user`
    # создаётся заново). Долгоживущий экземпляр — команда, скрипт — покажет
    ...
    def joined_year(self) -> int:
        ...
        Год, а не дата: точный день — лишние персональные данные на
        публичной странице. По алматинскому времени, а не по UTC: иначе
        зарегистрировавшимся в новогоднюю ночь профиль показывал бы
        прошлый год.
        """
...
class Genre(models.Model):
    ...
    строку в полосе на главной, то есть решение редакции. Сеется
    миграцией, правится в админке.

    `position` хранится — порядок жанров редакторский, а не алфавитный.
    Числа произведений здесь нет: это агрегат по каталогу, и колонкой он
    разошёлся бы с выдачей на первой же смене статуса работы.
    """
...
class Tag(models.Model):
    ...
    Жанр — полка, тег — то, о чём написано прямо сейчас. Отсюда открытый
    список, путь `pending → accepted | rejected` (BR-TAG-03) и блок-лист
    (BR-TAG-05).

    **Оба счётчика — производные** (DEC-53): считаются по `StoryTag` с его
    датой, и только по публичным работам — по счётчику читатель не должен
    догадываться, что у кого-то есть черновик с этим тегом.
    """
    ...
    def weekly_count(self) -> int:
        """Сколько раз тег поставили за неделю — «Осы аптада» (DEC-31).

        По дате связки, а не колонкой: хранимое число не убывает, и через
        месяц недельная витрина стала бы копией накопленной.
        """
        def counted():
...
class StoryTag(models.Model):
    """Связка «работа — тег», с датой (DEC-31).

    Голое M2M не несёт момент, когда автор поставил тег, — а без него
    «Осы аптада» нечем посчитать, и счётчик пришлось бы хранить.
    """
...
class Story(models.Model):
    ...
    Что здесь не хранится и почему — docs/architecture.md: число работ
    автора, счётчик жанра, давность правки и знак «участвует в байқау»
    считаются. Что хранится вопреки правилу — отмечено на месте.
    """
    ...
    # объём чтения и оси каталога комбинируются цепочкой.
    ...
    # «автор ещё не выбрал» — отдельное состояние, а не синоним «10+». На
    # детской платформе дефолт проставлял бы отметку за человека.
    ...
    # намеренно: агрегат по логу с окном считался бы на каждой странице
    # каталога. Инвариант `recent_views <= views`; убыли пока нет — журнала
    # просмотров с датами не существует.
    recent_views = models.PositiveIntegerField('14 күндегі оқылым', default=0)
    # Голоса за реакции по всем главам (BR-14, DEC-32) и комментарии. Оба
    # агрегата — колонки: у работы без текста глав нет вовсе, и вычисление
    # обнулило бы ей метрику в каталоге (BR-14a). `likes` двигает сам факт
    # голоса, а не смена его вида (`queries/story.toggle_chapter_reaction`).
    ...
    def chapters(self) -> int:
        """Сколько частей у работы — по записям глав (DEC-51).

        Не колонка с объявленным числом: обещание частей, которых никто не
        написал, портал не даёт. Выдача подставляет аннотацию
        `chapter_count`.
        """
    ...
    def has_chapters(self) -> bool:
        """Написана ли хоть одна глава (аннотация `has_any_chapter`).

        Не «есть ли текст»: глава с пустым телом даёт ноль знаков, но
        работа уже начата, и полоса внимания кабинета зовёт дописать
        именно её — а не начать то, что начато.
        """
    ...
    def text_chapter(self):
        ...
        пустой редактор: иначе автор заведёт вторую главу там, где текст
        один по определению.
        """
    ...
    def apply_moderation(self, outcome: str, reason: str = ''):
        ...
        статус без уведомления оставляет автора гадать, а уведомление без
        статуса обещает публикацию, которой не произошло. Правило живёт
        здесь, а не в `admin.py`: админка — сегодняшний инструмент
        модерации (DEC-23), завтра им будет вью.

        Причина обязательна у обоих отрицательных исходов (BR-11, BR-72b):
        «Толықтыру қажет» без неё сообщает столько же, сколько
        «Қабылданбады», то есть ничего.

        Решается только то, что автор сам отправил на проверку: готовность
        объявляет он (FR-WRITE-09), модератор отвечает «да» или «нет».
    ...
    def total_chars(self) -> int:
        """Объём написанного текста (аннотация `effective_chars`).

        Оценки по заявленным частям нет: ненаписанная работа честно
...
class Chapter(models.Model):
    ...
    Обратная связь называется `chapter_set`: имя `chapters` занято у
    `Story` числом частей, и подменять одно другим нельзя.
    """
...
class ChapterReaction(models.Model):
    ...
    Агрегат по `ChapterReactionVote` — колонка, а не подзапрос на каждый
    рендер главы: обновляется в момент голосования. Строка заводится
    первым голосом, а ряд из пяти кнопок полон и без неё.
    """
...
class ChapterReactionVote(models.Model):
    """Кто поставил какую реакцию на главе (BR-REACT-02/03).

    Одна активная реакция на пользователя и главу — ограничение базы, а не
    только формы: повторный клик снимает реакцию, клик по другой заменяет.
    `ChapterReaction.count` — агрегат по этой таблице, `Story.likes` — по
    числу голосов (BR-14a).
    """
...
class Contest(models.Model):
    ...
    Из них считаются фаза, отсчёт дней и год; заявки считаются по самим
    заявкам. Колонок `status`, `days_left`, `year`, `submissions` нет и
    заводить их нельзя: они протухают назавтра (BR-40a).

    Списки — номинации, этапы, жюри, условия — лежат отдельными таблицами
    и отданы наружу свойствами: шаблон перебирает список, а менеджер связи
    в шаблоне не перебирается. Свойства **кэшируются на экземпляре** —
    страница спрашивает состав по нескольку раз, а экземпляр живёт один
    запрос, и снимок не успевает устареть.
    """
    ...
    # По слагу, а не по совпадению имён: у выпусков имена расходятся.
    ...
    def phase(self) -> str:
        ...
        Фаза «қазылар қарауда» отдельная: между дедлайном и итогами
        конкурс не «активен» и ещё не «завершён».
        """
    ...
    def other_editions(self) -> list:
        ...
        Без них завершённый конкурс — тупик, хотя приём в выпуск этого
        года может идти прямо сейчас. Выпуск показывается карточкой,
        поэтому и выбирается как карточка (`for_card`).
        """
        if not self.series:
            return []
...
class ContestAward(models.Model):
    ...
    Набор произвольный: общего реестра номинаций нет и быть не может — он
    и есть то, чем один конкурс отличается от другого.

    `image` — эмблема, её грузит админ. Пусто — типографическая заглушка.
    Раму (медальон, кольцо, тень) рисует платформа: иначе через десять
    конкурсов ряд наград станет коллекцией чужих JPEG.
    """
...
class AwardGrant(models.Model):
    ...
    **Хранится сам акт**, а не список наград у автора: «Бас жүлде» из
    данных не вычисляется — это решение жюри, и этим конкурсные награды
    отличаются от системных знаков (BR-ACH-01). Автор не хранится: он у
    работы.
    """
...
class Submission(models.Model):
    ...
    Один автор — одна работа на конкурс; ограничение базы, а не только
    формы: вторая заявка ломает счёт участников и конкурсную биографию.
    Хранится дата подачи, подпись выводится (BR-41a).
    """
    ...
    # Ответы формы подачи (DEC-21, DEC-24) — пишутся один раз, дальше
    # только видны жюри и модератору.
...
class Follow(models.Model):
    ...
    Списки «Жазылулар» и «Оқырмандар» публичны, но входа в контент из них
    нет: читать зовут жинақтар и каталог (DEC-31).
    """
...
class Collection(models.Model):
    ...
    Создаёт **только редакция**; личное хранение — это «Кітапхана».
    Подборка отвечает на «зачем читать сейчас», поэтому имя у неё —
    фраза-состояние, а не жанр. Ни `count`, ни `covers` не хранятся.
    """
    ...
    def stories(self) -> list:
        ...
        Через `item_set.all()`, а не своим `select_related`: свой запрос
        игнорирует `prefetch_related` вызывающей стороны, и десять карточек
        на главной стоили бы десяти лишних запросов.
        """
...
class LibraryEntry(models.Model):
    ...
    Три вида — «сақталған», «оқу үстінде», «оқылған» — **не пересекаются**:
    работа лежит ровно в одном, и это ограничение базы.

    Номера главы здесь нет (DEC-52): где читатель остановился, знает
    `ReadingProgress`, а полка получает номер аннотацией `progress_chapter`.
    Вторая колонка о том же факте однажды разошлась с первой.
    """
...
class ReadingProgress(models.Model):
    ...
    сообщает, где закрыли страницу. Когда появится, поле уступит ей место.
    """
...
class StoryComment(models.Model):
    ...
    Вложенность **ровно одна**: ответ на ответ даёт дерево, которое на
    телефоне не читается и которое некому модерировать. Время хранится
    моментом, подпись выводится (BR-70a).
    """
    ...
    # Агрегат по `CommentLike` — колонка, а не подзапрос на каждый
    # комментарий страницы: пересчитывается в момент нажатия, не на чтении.
    ...
    def replies(self) -> list:
        """Ответы на комментарий, по `pk` (`Meta.ordering`).

        `cached_property`: метку `.liked` (BR-31) ставит `queries/story` на
        уже полученных объектах, и второй вызов новым запросом вернул бы
        другие объекты без неё. `reply_set.all()` без `select_related`:
        второе рвёт кэш `prefetch_related('reply_set__author')` и делает из
        одного запроса на страницу по запросу на комментарий.
        """
...
class ChapterPoll(models.Model):
    ...
    Не квиз: правильного ответа нет и очков не бывает — есть повод
    вернуться у читателя и обязательство дописать у автора. Опрос
    закрывается публикацией следующей главы (BR-POLL-05), поэтому `closed`
    вычисляется.
    """
...
class PollVote(models.Model):
    ...
    Одна ставка на весь опрос, не на вариант, и после отправки не
    меняется — ограничение базы. Смену голоса BR-POLL не оговаривает.
    """
...
class Notification(models.Model):
    ...
    **Хранится «когда», выводится «как давно»** (BR-70a): подпись и группа
    считаются из момента, колонками они устаревали бы назавтра.

    **Уведомление ведёт к своему предмету** (BR-72a): имя конкурса или
    работы приходит из объекта, а в `text` лежит только событие.
    Исключений два, и оба про чужие слова: цитата читателя у комментария и
    причина модератора у отказа (BR-11).
    """
    ...
    # состояние работы. Из `Story.status` он не выводится — статус живёт
    # дальше события, и вчерашний отказ сказал бы «Модерацияда».
