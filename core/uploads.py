"""Растровые файлы: проверка содержимого и пережатие.

Чистые функции без импорта моделей — их зовёт `core/models/` (валидатор
поля) и `media_cleanup.py` (пережатие в сигнале). Импорт моделей здесь
дал бы цикл: `core/models/` импортирует `validate_raster_image` отсюда же.
...
навсегда. Уборка старого файла — в
...
def validate_raster_image(file) -> None:
    """Растр и только растр, теперь по содержимому, а не по имени.
    ...
    if ext not in RASTER_FORMATS:
        # Пометка правила — читателю коду, не
        # автору-ребёнку в тосте. `` в самом сообщении здесь не
...
def resize_raster_image(file, ext: str) -> ContentFile:
    """Пережать в границу стороны и в формат по расширению.
