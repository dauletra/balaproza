"""Раскладка: числа, которые обязаны совпадать в разных файлах.

Ширина колонки, брейкпоинт рейла, размер карточки и отступ сетки живут в
десятке шаблонов классами Tailwind. Разъезжаются они молча: рейл,
переключённый не на том брейкпоинте, что его мобильный дубль, даёт либо
дырку в контенте, либо один и тот же блок дважды на экране, — и оба
исхода видны только на устройстве нужной ширины.

Числа объявлены здесь один раз, и тест сверяет с ними разметку.
"""
...
# Путь объявлен в `test_lint`, а не здесь.
from core.tests.test_lint import TEMPLATES_DIR
