...
def tiny_image(name: str = 'cover.png') -> SimpleUploadedFile:
    ...
    аватара и эмблемы. `validate_raster_image` декодирует файл
...
def user(**over) -> User:
    ...
    Онбординг завершён (`terms_accepted_at`): фабрика заводит уже
...
def genre() -> Genre:
    """Любой жанр из справочника. Свой не заводим: список закрыт."""
...
def story(*, author=None, chapters: int = 0, chars: int = 1200,
    ...
    отдельно от текста больше не существует, и «сериал на три
    бөлім» в тесте означает три записи с телом.

    `published` — прошли ли эти главы модерацию. По умолчанию да, и
...
def submit(story_obj) -> None:
    ...
    Статус пересчитывается здесь же: «в очереди» — это состояние
...
def contest(*, phase: str = 'accepting', **over) -> Contest:
    ...
    Фаза не хранится — она выводится из трёх дат, поэтому тест
