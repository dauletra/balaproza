...
class PagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_home_guest(self):
        """Восемнадцать: ряды, жинақтар, книга недели, полоса жанров, две
        витрины тегов, баннер конкурса, секция «Байқаулар», счётчики
        масштаба и новые имена.

        **Было пятнадцать, и три добавлены осознанно.** Это тот случай,
        когда бюджет растёт правильно: раньше три ряда и счётчики
        обслуживались одной выдачей — `list()` по **всему** публичному
        каталогу, который дальше резался в Python. Один запрос вместо
        трёх, и цена его растёт вместе с каталогом; на тысяче работ
        главная стала бы самой медленной страницей портала.

        Теперь три ограниченных запроса по пять карточек (`home_rows`) и
        три `COUNT` под счётчики масштаба, закэшированные на пять минут.
        Шесть вместо трёх — и ни один больше не зависит от размера
        каталога. Кэш здесь холодный намеренно: бюджет меряет то, что
        увидит первый посетитель.

        Прежде было двадцать два, и семь ушли тремя правками. Две из них
        в силе:
        ...
        **Два.** Карточка жинақа показывает три обложки, а состав
        ...
        Третья — «счётчики масштаба считаются по спискам, которые
        страница уже держит» — **отменена сознательно**: она экономила
        два запроса ценой того самого списка на весь каталог. Дешевле
        оказалось не держать список.
        """
    ...
    def test_catalog(self):
        ...
        Счётчик пресета обязан быть настоящим — неправдивый хуже
    ...
    def test_catalog_signed_in(self):
        """Метки читателя на карточках не стоят ни одного запроса
    ...
    def test_search(self):
        """Поиск — `?q=` на `/catalog/`, тот же движок и тот же
    ...
    def test_story_page(self):
        ...
        Двадцать три: прочтение пишется строкой в журнал, иначе
    ...
    def test_genre_index(self):
        ...
        **Было два, стало пять — осознанно.** Число работ над сеткой было
        суммой счётчиков жанров, и работа с основным и дополнительным жанром
        входила в неё дважды: «32 шығарма» при 21 в каталоге. Теперь это
        `portal_stats`, то же число, что в хиро главной: три `COUNT`,
        закэшированные на пять минут и общие с главной. Кэш холодный, как и
        у главной: бюджет меряет первого посетителя."""
    ...
    def test_sitemap(self):
        """`sitemap.xml`: по два запроса на раздел — паджинатор Django
        сначала считает, потом выбирает, — плюс один за ссылками школы
        (глобальный контекст-процессор). `StaticViewSitemap` в базу не
        ходит.

        Разделов шесть: работы, авторы, жанры, теги, жинақтар, конкурсы.
        Было три запроса на один раздел; пять новых — это длинный хвост
        поиска, ради которого сайт и собран серверным рендерингом.

        Тринадцать на страницу, которую читает краулер раз в сутки, —
        честная цена. Дороже было бы брать выдачи витрин: у каждой свои
        prefetch'и под карточку, и `all_collections()` тянул бы обложки
        трёх работ на каждую подборку. Поэтому у карты свои выборки
        (`data.sitemap_*`), голыми слагами.
        """
...
class ModerationPagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_the_queue_does_not_grow_with_the_line(self):
        ...
        Семь, не шесть: плюс один `COUNT` за бейдж открытых жалоб в шапке
         — тот же счётчик у `/moderation/reports/`.
...
class PersonalPagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_story_page_signed_in(self):
        ...
        прочтение пишется строкой в журнал. Полка здесь не
        заводится: первый заход показывает тизер, то есть продвижения ещё
        нет. Она обойдётся в четыре запроса на той главе, до
    ...
    def test_manage_story(self):
        """Страница одной работы в кабинете: сама работа, главы и
        редактор активной главы на одном экране (11.1b), чек-лист
        готовности и полоса внимания.

        Разделение ревизий добавило к каждой выборке глав их
        историю — состояние главы («тексеруде», «өзгертілген») и ответ на
        «есть ли что подавать» живут в ней. Ревизии приезжают
        `prefetch`'ем, а не запросом на главу: без него страница автора с
        двадцатью бөлім стоила бы двадцать запросов.

        16, не 18: `missing` теперь выводится из уже посчитанного
        `checklist` (`_workspace_context`), а не отдельным
        `missing_for_review` — тот зовёт тот же `publish_checklist`
        (`has_chapters`, `tags.exists()`), что и чек-лист панели, и
        двойной счёт на одном показе был чистым дублированием, не новой
        ценой за новый блок. Последние два — очередь и решение
        модератора: замечание висит на этой странице до
        повторной отправки, и помнить его наизусть, пока правишь, не
        работа автора.
        """
        login_as(self.client)
    ...
    def test_chapter_editor(self):
        """Редактор главы (11.1b): то же тело, что у `manage_story`, —
        список глав и чек-лист рядом с редактором, а не два отдельных
        шаблона с половиной разметки, повторённой один в один.

        Было 6 (сама глава берётся по `pk` через `chapter_by_id` — одним
        `select_related` вместо `chapter_of` + `poll_of`), стало 15: +9
        на список глав и чек-лист, которых у отдельной страницы
        редактора раньше не было вовсе. Готовность кнопки «Модерацияға
        жіберу» (V14) при этом считается без второго прохода
        `publish_checklist` — см. `test_manage_story`."""
        login_as(self.client)
