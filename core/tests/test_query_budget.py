...
class PagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_story_page(self):
        ...
        Стало 23 — цена засчитанного оқылым, и она вся в первом заходе
        гостя. Один запрос из пяти это сам счётчик (`UPDATE core_story`),
        остальные четыре — заведение сессии: проверка ключа, вставка и
        пара savepoint'ов вокруг неё. Сессии у гостя до этого не было
        вовсе, потому что портал ничего о нём не помнил.
...
class PersonalPagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_story_page_signed_in(self):
        ...
        Тридцать четыре — первое открытие работы этим читателем:
        заводится закладка, а вставка тянет за собой транзакцию, и
        прочтение пишется строкой в журнал (DEC-55). Полка здесь не
