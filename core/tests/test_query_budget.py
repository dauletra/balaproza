...
class PersonalPagesStayWithinTheirQueryBudget(TestCase):
    ...
    def test_manage_story(self):
        ...
        Последние два — очередь и решение модератора (BR-80): замечание
        висит на этой странице до повторной отправки, и помнить его
        наизусть, пока правишь, не работа автора.
        """
        login_as(self.client)
