...
class TestReferencedPathsExist(unittest.TestCase):
    ...
    ALLOWED_MISSING = {
        ...
        'local_settings.py',
        # Удалён на этапе 11 Ф14. ТЗ говорит о нём в прошедшем времени —
        # история решений и есть ценность реестра, а проверка «файл на
        # диске» не отличает рассказ о прошлом от ссылки на настоящее.
        'core/stub_data.py',
    }
...
class TestDataContract(unittest.TestCase):
    ...
    Спрашивается фасад, а не конкретный файл. В этом весь смысл Ф14:
    хелпер переехал из стаба в менеджер модели, и проверка «есть ли такая
    строка в таком-то файле» падала бы на каждом успешном шаге миграции —
    то есть требовала бы, чтобы код не двигался.
    """
