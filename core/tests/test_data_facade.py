...
# Кому можно знать про `stub_data`: самому стабу и команде сида. Фасад из
# этого списка ушёл — все разделы читают модели, и стаб остался только
# источником демо-корпуса. Сид читает его напрямую, потому что он и есть
# конвертер между двумя мирами: через фасад он читал бы то, что сам
# записал. Тесты каталогом ниже не проверяются — стаб-корпус пока и есть
# предмет их сверки.
STUB_IMPORTERS_ALLOWED = {'stub_data.py', 'seed_demo.py'}
