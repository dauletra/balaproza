...
# Кому можно знать про `stub_data`: самому стабу, фасаду и команде сида.
# Сид — конвертер между двумя мирами, и читать он обязан исходный: фасад
# по мере Ф14 начинает отвечать моделями, и сид через него читал бы то,
# что сам записал. Тесты каталогом ниже не проверяются вовсе — стаб-корпус
# пока и есть предмет их проверки.
STUB_IMPORTERS_ALLOWED = {'stub_data.py', 'data.py', 'seed_demo.py'}
