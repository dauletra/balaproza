...
«Можно подать» и «конкурс не завершён» — разные вопросы: кнопку
«Қатысу» решает `is_accepting`, знак «Байқауға қатысады» — `not
is_finished`. Одним словом «активен» они не называются.

Форма ничего не отклоняет: у кандидатов бывают заметки, но решение
принимает человек.
"""
...
def contest_participants(contest) -> list:
    """Работы конкурса, доступные читателю: accepted + победители.
...
def open_contests() -> list:
    ...
    трёх дат, и `ORDER BY` её не выражает."""
...
def has_submission(user, contest_slug: str) -> bool:
    """Один автор — одна работа на конкретный конкурс."""
...
def busy_contest_of(user, story_slug: str, *, besides: str = ''):
    """Незавершённый конкурс, который уже держит эту работу.
...
def can_withdraw(user, contest) -> bool:
    """Можно ли отозвать заявку: пока идёт приём и жюри не вынесло
...
def common_rules(contest) -> list:
    """Правила, действующие на любом конкурсе; в условия отдельного
    ...
    Возраста здесь нет и быть не может — его ставит конкурс.
    """
...
def submission_checklist(story, contest, *, chars: int = None) -> list:
    ...
    когда конкурс ставит вилку. «Объём» — единственная авто-проверка,
...
def submission_candidates(user, contest_slug) -> list:
    """Работы автора как кандидаты и что о них стоит знать.
    ...
    черновик нельзя ни дать жюри, ни показать рядом с победителями.
...
def create_submission(user, contest, story, *, ai_declaration: str,
    ...
    """Новая заявка. Возвращает `(submission, created)`.
...
def withdraw_submission(user, contest) -> bool:
    """Отзыв заявки. Условие проверяется здесь заново:
...
def contest_history(user, *, is_self: bool = False) -> list:
    """Конкурсная биография автора, свежие сверху.

    Правило приватности живёт здесь, а не в шаблоне: публично
