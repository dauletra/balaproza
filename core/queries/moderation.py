"""Очередь модератора и всё, что нужно для решения, — и
...
статусе: статус выводится по главам и у публичного сериала остаётся
...
def submitted_chapters(story) -> list:
    """Поданные главы с текстом и сравнением.
...
def decision_history(story) -> list:
    """Прошлые решения по работе — и модератору, и автору.
...
def claim_story(story, moderator):
    """«Взял в работу»: метка на себя, чужую не перебиваем."""
...
def create_report(reporter, *, story=None, comment=None, reason: str,
    ...
    как и у остальных действий по слагу. Ровно одна цель, причина
...
def resolve_report(report, moderator, *, action: str, reason: str = '') -> None:
    ...
    ревизия, а уже опубликованное (`Story.take_down`); у комментария
