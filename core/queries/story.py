...
def record_story_view(story) -> None:
    ...
    Оба счётчика растут вместе. `views` — накопленное за всё время,
    `recent_views` — ось «Қазір танымал» (DEC-36); **убыли у второго пока
    нет**, потому что убывать не от чего: журнала просмотров с датами в
    базе не существует, и окно в четырнадцать дней остаётся обещанием.
    Отдельная задача — план оптимизации, §6, вопрос 4.

    Через `update()`, а не `save()`, по двум причинам: гонки двух
    читателей складываются в базе, а не затирают друг друга, и `auto_now`
    у `updated_at` остаётся нетронутым — чтение работы не есть её правка,
    и «өзгертілген бүгін» после чужого захода было бы неправдой.

    Объект в памяти двигается следом: страница показывает счётчик тут же,
    и цифра, отставшая на один заход, читается как «мой заход не
    засчитали».
    """
