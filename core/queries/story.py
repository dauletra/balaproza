...
**Ряд реакций всегда полный**: нулевые не выбрасываются —
...
def chapters_of(story_slug: str, *, as_author: bool = False) -> list:
    ...
    Читателю отдаются **только опубликованные**: у главы, чья
 ...
 тем же join'ом приезжает и опубликованная ревизия.
    """
...
def _attach_my_reaction(chapter, viewer):
    """Ставит `chapter._my_reaction` — голос текущего читателя.
...
def reactions_of(chapter, viewer=None) -> list:
    ...
    текущий читатель. У главы из `chapter_of(..., viewer)`
...
def toggle_chapter_reaction(chapter, user, kind: str) -> str:
    """Ставит, снимает или меняет реакцию на главе.
 ...
 поэтому смена вида его не трогает. Возвращает новый slug
    реакции, '' — если снята.
...
def poll_of(story_slug: str, chapter_number: int, viewer=None):
    """Опрос главы или None: опрос необязателен."""
...
def cast_poll_vote(poll, user, option_slug: str) -> bool:
    ...
    Закрытый опрос и повторный голос — no-op.
...
def _attach_liked(comments: list, viewer) -> list:
    """Проставляет `.liked` на каждый комментарий и его ответы.
...
def top_level_comment_of(story_slug: str, comment_id) -> StoryComment | None:
    """Комментарий этой работы, но только верхнего уровня — на
...
def record_story_view(story, viewer=None) -> None:
    ...
    Строка нужна для убыли: без дат окно в четырнадцать дней не
    убывало, и «Қазір танымал» со временем сходилась с «Ең көп оқылған».
 Считать по журналу на каждой странице каталога дорого,
...
def add_comment(story, author, *, text: str, chapter_number=None, parent=None) -> StoryComment:
    """Новый комментарий или ответ. Валидность `parent`
...
def delete_comment(comment) -> None:
    """Удаляет комментарий вместе с его ответами (каскад; уровень
...
def toggle_comment_like(comment, user) -> bool:
    """Лайк комментария — toggle: повторный клик снимает.
...
def book_of_week():
    """Выбор редакции на эту неделю или None.
