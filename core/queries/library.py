...
Продолжение чтения — первоклассный сценарий: им живут и хиро
...
def progress_chapter_subquery(user_ref: str = 'user', story_ref: str = 'story'):
    ...
    разошёлся. Без записи — первая глава: работа на полке, но не
    открыта.
    """
    return Coalesce(
        Subquery(
...
def record_reading_progress(user, story, chapter_number: int, chapters) -> None:
    """Запомнить, где читатель остановился.
...
def move_to_shelf(user, story, *, finished: bool, advanced: bool) -> None:
    """Автопереход полки по факту чтения.
    ...
    запись. Повторное чтение дочитанной возвращает её на «оқу үстінде»:
    строка `done` предлагает «Қайта оқу», и после нажатия она обязана
    описывать то, что происходит.

    **Полку трогает только продвижение** (`advanced`) — глава, отличная от
    той, на которой стоит закладка. Открытие, показывающее ровно то место,
    где читатель и был, не двигает ничего: снятие кнопкой, которое правила
    прямо разрешают, редиректит на эту же страницу, и запись воскресала бы
    раньше, чем читатель увидит результат нажатия. По той же причине взгляд
    на страницу не перебивает ручное «сақталған» на `reading`.
    """
    if not advanced:
        return
