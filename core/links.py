...
def catalog_default_sort(mode: str) -> str:
    """Дефолтная сортировка режима: каталог (в т.ч. с `?q=`) и жанр —
    «Қазір танымал», тег — «Жаңалары». У тега роль самой быстрой оси
    портала, и там ценна свежесть, а не набранные просмотры."""
...
class CatalogState:
    ...
    туда popularity и ломать порядок тега.
    """
    ...
    def href(self, **over) -> str:
        """Канонический URL этого состояния с изменёнными осями.
        ...
        if st.genre:
            ...
            # `?q=` едет сюда же — у поиска больше нет своего пути.
    ...
    def canonical_href(self) -> str:
        """Адрес, под которым эту выдачу стоит знать поисковику.

        Сам раздел, без осей и без запроса: `/catalog/`, `/genres/<slug>/`,
        `/tag/<slug>/`. Оси комбинируются по шесть штук плюс страницы, и
        каждая комбинация — отдельный адрес с почти тем же содержимым; на
        молодом домене краулер тратит обход на них вместо самих работ.

        Сортировка тоже не каноническая: «по алфавиту» и «по популярности»
        — один и тот же список в другом порядке.
        """
        return CatalogState(mode=self.mode, genre=self.genre,
                            tag=self.tag).href()

    @property
    def is_narrowed(self) -> bool:
        """Сужена ли выдача — то есть стоит ли закрывать её от индексации.

        Запрос считается сужением наравне с осями: страница результатов
        поиска по чужому слову в индексе не нужна никому, а плодится она
        быстрее всего.

        Жанр и тег сужением **не** считаются: они путь, а не фильтр, и у
        каждого свой канонический адрес, который и должен индексироваться.
        Поэтому здесь `self.axes`, а не `active_count` — тот считает
        включённые чипы для бейджа на кнопке сүзгі и жанр с тегом в них
        включает.

        Страница не спрашивается: её знает пагинатор, а не состояние, и
        решают её там же, где рендерят.
        """
        return bool(self.query or self.sort or any(self.axes.values()))
...
def attention_links(user) -> list:
    """Сигналы кабинета с готовыми ссылками.
...
def checklist_links(story) -> list:
    """Пункты чек-листа с готовыми ссылками.
    ...
    Пункты, ведущие в баптаулар, несут ещё и якорь своего поля: без него
    ссылка отправляла на верх `/settings/` целиком, а панель обещала
    ровно обратное. `id` совпадает с тем, что
...
def notification_href(n) -> str:
    ...
    авторском кабинете** (публично её может не быть).

    Здесь, а не в шаблоне: этот же адрес нужен вью — клик проходит через
    `notification_open`, снимающее «непрочитано». Пусто — предмета
