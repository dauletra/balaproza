...
class StoryTagInline(admin.TabularInline):
    """Теги работы. `tags` — M2M через `StoryTag` (Ф15) и потому
    ...
    fieldsets = (
        ...
        ('Модерация', {
            ...
            'description': 'Модерация шешімі — тізімдегі әрекет арқылы: '
                           'сонда ғана автор хабарлама алады. '
                           '«Мәртебесі» енді бөлімдерден есептеледі: '
    ...
    def save_model(self, request, obj, form, change):
        ...
        if left_moderation:
            self.message_user(
                ...
                'Модерация шешімін тізімдегі әрекет арқылы қабылда.',
                messages.WARNING)
