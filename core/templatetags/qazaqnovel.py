"""Шаблонные хелперы Qazaqnovel — подключаются через {% load qazaqnovel %}.
...
@register.filter(name="start_label")
def start_label(story):
    """Подпись главного CTA витрины: что именно откроется по нажатию.

    «Оқуды бастау» честна для одночастной работы — открывается она сама.
    У сериала то же слово не говорило, куда попадёшь: к списку частей, к
    последней или к первой. Называем часть.

    Минут здесь нет намеренно: рядом стоит `reading_meta`, и «7 минут оқу»
    на кнопке было бы дословным повтором строки над ней.
    """
    return "Оқуды бастау" if story.is_single else "1-бөлімді оқу"
...
def update_meta(story):
    ...
    сколько человек его открывало — это уже сказал соседний ряд «Көп
    оқылған». Дата — когда читателю показали последнюю часть
    (`last_published_at`), а не когда автор трогал запись: `updated_at`
    двигает любое сохранение, включая решение модератора о чём угодно.

    `ago`, а не `since`: для читателя выход новой части — событие, а не
    его собственная работа (docs/ui.md).
    """
    when = ago(story.last_published_at)
    chapters = f"{story.chapters}-бөлім"
    return f"{chapters} · {when}" if when else chapters
