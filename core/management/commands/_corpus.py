...
дат, и абсолютные литералы протухают молча. Завершённый конкурс,
...
TAGS = [
    ...
    # pending — для иллюстрации работы модерации
...
class Author:
    username: str   # без @
    pen_name: str   # публичное имя автора — другого портал не хранит
    bio: str
    # Когда пришёл на портал. Днями, а не годом: ось «Жаңа есімдер» — это
    # возраст аккаунта, и 1 января превращало бы в новичка того,
    # кто зарегистрировался одиннадцать месяцев назад, и наоборот.
    # Приход не позже первой работы: это проверяется тестом.
    joined_days_ago: int


AUTHORS = [
    Author("rudazov",   "Rudazov",       "Фэнтези, шытырман", 1050),
    Author("aygerim_k", "aiqalam",       "Жас прозаик · Алматы", 430),
    Author("bekzhan_t", "BekTor",        "Қалалық әңгімелер", 540),
    Author("dina_books","dina.books",    "Балалар әдебиеті", 300),
    Author("sayyn",     "sayyn",         "Фантастика, шытырман", 620),
    # Демо-пользователь, под которым логинимся через фейк-сессию (см. core.views.login_view).
    Author("aidana",    "aidana",        "Жас прозаик · Тараз", 160),
]
...
class Story:
    ...
    views: int
    # `likes` не заводится здесь: агрегат по реакциям должен
    # считаться из `Chapter.reactions`, а не вписываться отдельным
    # числом — иначе оно молча расходится с настоящими реакциями главы
    # (было расхождение у 19 из 23 работ).
    comments: int
    # Сколько строк журнала завести внутри окна: колонка
    ...
    recent_views: int = 0       # ось «Қазір танымал»
    # Переход в публичное — только через модерацию.
    ...
    tags: tuple = ()            # до 10 слагов на произведение
    # Пусто — «автор ещё не выбрал», отдельное состояние, а не
    ...
    # «Танымал тегтер». Создание не раньше прихода автора.
...
STORIES = [
    ...
    # Черновик: бейдж «Жоба» и сигнал «начата, но ни одного бөлім»
...
CHAPTERS_BY_STORY: dict = {
    ...
    # Работа на модерации обязана нести текст: модерируется поданная
    # ревизия, а не статус, и работы на проверке без единой главы
    # на портале больше не бывает. Фикстура «начат, но ни одного бөлім»
    # осталась за черновиком `aidana-kus`.
    "aidana-erteg": [
        _chapter("aidana-erteg", 1, "Толық мәтін"),
    ],
    "temniy-lord": [
    ...
    "dalney-berega": [
        # Реакции — на главу, не на произведение.
...
class StoryComment:
    ...
    # и «2 сағат бұрын» пришлось бы разбирать обратно в момент.
    ago: timedelta
    text: str
    likes: int = 0
    is_author_badge: bool = False
    liked: bool = False             # текущий пользователь поставил лайк
    replies: tuple = ()             # один уровень вложенности; tuple[StoryComment]
...
COMMENTS_BY_STORY: dict = {
    ...
    # «Автор», нить с одним и двумя ответами — уровень всегда один.
    "dalney-berega": [
        ...
        # ветка «свой» в меню (Жою вместо Шағым) не видна нигде.
        StoryComment(
...
@dataclass(frozen=True)
class Collection:
    """Редакционная подборка — первичный вход в чтение."""
    slug: str
    name: str
...
@dataclass(frozen=True)
class ContestAward:
    """Номинация конкурса и её награда. Общего реестра номинаций
...
@dataclass(frozen=True)
class Contest:
    """Конкурс: три даты, из которых выводится всё остальное."""
    slug: str
    name: str
    ...
    # Файл в MEDIA_ROOT, грузит админ. Пусто — платформа рисует
    # типографическую афишу; в корпусе пусто у всех.
    poster: str = ""
    # Слаг «семейства» — повторяющегося конкурса, у которого бывают
    # выпуски разных лет. Пусто — разовый конкурс.
    series: str = ""
    description: str = ""
    conditions: tuple = ()       # bullet points
    timeline: tuple = ()         # TimelineStage[]
    jury: tuple = ()             # JuryMember[]
    # Пороги объёма для подачи (знаки)
    min_chars: int = 5_000
    max_chars: int = 15_000
    # Возрастное требование конкурса: любая граница может быть None.
    # Здесь, а не в правилах платформы: своего ценза у неё нет.
    min_age: Optional[int] = None
    max_age: Optional[int] = None
    # Номинации этого конкурса. Показываются ДО итогов: «вот что
...
CONTESTS = [
    ...
    Contest(
        ...
        # языка, этот конкурс сужает до одного.
        conditions=(
            "Тек қазақ тілінде",
        ),
        timeline=(
    ...
    Contest(
        ...
        # ставит. Без такого выпуска состояние негде увидеть.
        description=(
    ...
    # Второй выпуск того же конкурса — иначе связь выпусков нечем
...
@dataclass(frozen=True)
class AwardGrant:
    """Присуждение: хранится сам акт — решение жюри из данных не
...
@dataclass(frozen=True)
class Submission:
    """Заявка автора. Хранится дата подачи, подпись выводится.
...
SUBMISSIONS_BY_USER: dict = {
    ...
    "dina_books": [
        ...
        # в списке страницы конкурса до объявления итогов.
        Submission(
...
# ───────────────────────── Авторлар мектебі — внешние ссылки ───────────────
...
class Notification:
 ...
 имя предмета берётся у самого предмета, а в `text`
...
NOTIFICATIONS_BY_USER: dict = {
    "aidana": [
        ...
        # сейчас. Сколько осталось, знает конкурс: числа в тексте нет.
        Notification(
        ...
        # Возврат на доработку с замечанием. Замечание в `text` —
        # собственные слова модератора, то же исключение, что и
