...
Жёсткий CSP выставляется на уровне веб-сервера при первом деплое, и
составлять его надо **по факту**, а не по намерению. Фронт почти весь
self-hosted, но исключения есть, и их три: страница входа грузит
`telegram.org/js/telegram-widget.js`; `base.html` содержит инлайновый
`<script>` для тостов; Alpine в обычной сборке вычисляет выражения через
`new Function()`, то есть требует разрешения на исполнение строк.
Инлайн-стили тоже есть — OKLCH-цвета жанров. Наружу уходит ещё один
адрес, но не из браузера: `SENTRY_DSN` зовёт сервер, а не страницу.
