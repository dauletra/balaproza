...
Колонка «Stub source» сохранена намеренно: по ней видно, что чем стало, и она же объясняет, почему у некоторых моделей поля выглядят странно (`Story.likes`, `Tag.weekly_count` — счётчики без строк под ними, потому что в демо-корпусе их некому создать). **`User.followers` из этого списка вышел**: колонка осталась (её читают сортировка «Жаңа авторлар» и ось каталога — это `ORDER BY` и `WHERE`), но пересчитывается по строкам `Follow` — и в `toggle_follow`, и в сиде. Раньше сид клал туда число из корпуса, и профиль объявлял 8 420 оқырман при трёх записях.
