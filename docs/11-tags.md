...
| **HOME** | Секция «Тегтер» + виджет в правом рейле | две полосы: «Осы аптада» (`trending_tags`, по датам связок за 7 дней) и «Танымал тегтер» (`popular_tags`, по `usage_count`). В потоке — `partials/home/popular_tags.html` под `xl:hidden`, в рейле — `partials/right_rail/home.html` (FR-HOME-08/11) |
...
- `popular_tags(limit=10)` — сортировка по числу публичных работ с тегом; счётчик не хранится (DEC-53)
...
| Свежесть | обновляется сам, недельный срез показывает всплески | снимок вкуса редакции на дату сборки |
...
Обратное тоже верно: подборка стареет, а недельный срез — нет. Отсюда полоса «Осы аптада» на главной (FR-HOME-08) и сортировка `recent` по умолчанию на `/tag/<slug>/` — тег должен показывать движение, а не архив.
