...
| Тесты | `core/tests/` (21 файл, 1073 теста) | Контракт поведения, описан в [15](15-testing-contract.md) |
...
| FR-CAT-15 (ось «Түрі») | `CATALOG_KIND_FILTERS`, `KIND_PREDICATES` | `test_catalog.KindReplacesFormatAndStatus` |
| BR-10a (сериал всегда размечен) | правило в `STORIES` | `test_catalog.SerialCompletionIsAlwaysKnown` |
| `PUBLIC_STATUSES` вместо литерала | `home`, `search_index_json` | `test_catalog.PublicStatusesAreNotSpelledOut` |
...
## 14.9a MOD · Модерация (Django admin, DEC-23)

Кастомного UI до V2 нет, значит всё, чего нет на страницах портала, обязано делаться в админке. Файл — `core/admin.py`, тесты — `test_admin.py`.

| Задача модератора | Где | Тест |
|-------------------|-----|------|
| Решение по отправленной работе (BR-11, BR-72b) | действия `approve` / `send_back` / `reject` → `Story.apply_moderation` | `ModerationChangesStatusAndTellsTheAuthor`, `ModerationThroughTheAdmin` |
| Причина отказа | промежуточная страница `admin/core/story/moderation.html` | `test_empty_reason_does_not_apply` |
| Путь тега (BR-TAG-03) | групповые действия `accept` / `reject` у `TagAdmin` | — |
| Состав жинақ (DEC-31) | `CollectionItemInline`, отсортирован по `position` | — |
| Афиша и эмблема награды (BR-46, BR-47a) | `FileField` + `RASTER_ONLY`; раскладка `awards/<contest>/<award>.<ext>` | `MediaUploadsTakeRasterOnly` |
| Текст главы | `ChapterAdmin` — форм автора нет до Ф15 | `ChapterTextIsEditable` |

**Статус меняется действием, а не полем.** Правка «мәртебесі» в форме ничего не сообщает автору: работа уходит из очереди, и с его стороны не происходит ничего. Поле осталось редактируемым — «Аяқталды» у дописанного сериала ставить больше негде, — но такая правка сопровождается предупреждением (`StoryAdmin.save_model`).

**Решается только то, что автор отправил сам.** Одобрить чужой черновик значит опубликовать работу, которую не показывали: готовность объявляет автор (FR-WRITE-09).

**Чего в админке нет намеренно:** библиотека, прогресс чтения и подписки. Это личные записи читателя, модератору в них делать нечего, а список чужих полок — витрина персональных данных. Уведомления показаны только на чтение: их пишет событие.
...
| Действие модерации или `Story.apply_moderation` | [08 BR-11 / BR-72b](08-business-rules.md), [19 §19.4](19-f14-migration-plan.md) — статус и уведомление меняются вместе |
| Регистрация модели в `core/admin.py` | [10 DEC-23](10-resolved-decisions.md) — админка обязана уметь всё, чего нет в интерфейсе; личные записи читателя в неё не заводятся |
