...
итог — в правилах BR-76…BR-89 и решениях DEC-66…DEC-77.

Что из этого поменяло модель, а не экраны:

- **единица публикации — глава, точнее её ревизия** (BR-79): у главы есть
...
- **модерация — раздел `/moderation/`**, а не действия админки (BR-82);
- **список глав, редактор и чек-лист — одно рабочее место**
  (`/write/<slug>/`), а не четыре отдельных экрана: узкая колонка списка
  рядом с редактором на широких экранах, вкладки на мобильном (DEC-77).
...
**Telegram Login Widget с проверкой подписи закрыт** (NFR-25, BR-90,
DEC-78) — провайдер личности настоящий, демо-входа больше нет.
**Username-редактирование закрыто** (BR-91, DEC-79): плейсхолдер
`id<6 цифр>` меняется на `/me/edit/`, смена без истории и редиректа —
старый адрес отвечает 404.
...
онбординга, `age` заменён на `birth_date` (DEC-82). Заодно убран
дублирующий `/auth/signup/` — вход и регистрация всегда один экран,
один и тот же виджет (DEC-83). Впереди — то, что делается один раз при
...
- **Модель не собирает подписи.** «Сколько времени назад», фраза конкурса,
  «N бөлім» — фильтры `templatetags/balaproza`, правило записи — в `domain`.
...
- **Читателю отдаётся опубликованная ревизия, а не глава** (BR-79).
...
- **`Story.status` не присваивают** — его считает `refresh_status()` по
  главам (BR-79). Присвоенный руками держится до следующего пересчёта.
...
- **Видимость работы решает одна дверь — `story_by_slug(slug, viewer)`**
  (BR-76), и по умолчанию она закрыта до публичного. Действия по слагу
