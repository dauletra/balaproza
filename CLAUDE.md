...
│   └── tests/                    # 941 тестов в 16 файлах (см. ниже)
├── templates/
...
```
uv run python manage.py test core       # все 941 тестов
...
- **Исход модерации хранится** (BR-72b): `Notification.outcome` — `approved`/`rejected`/`''`. Из `Story.status` он не выводится: статус живёт дальше события, и вчерашний отказ завтра сказал бы «Модерацияда». Подпись — только через `MODERATION_OUTCOME_LABELS` (docs/16 §16.3b), в шаблоне слово не собирается
