...
DEC-78) — провайдер личности настоящий, демо-входа больше нет. Осталось
только то, что делается один раз при деплое: зарегистрировать бота и
указать ему прод-домен (`README.md`, раздел «Вход»).

Впереди: username-редактирование в профиле (случайный `id<6 цифр>` при
первом входе — своя, ещё не сделанная задача, BR-90).
