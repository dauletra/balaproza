...
**Переделка авторского флоу по аудиту закрыта** — разбор, все 11 этапов
(последний — подшагами 11.1a–11.4) и живая проверка в
[`AUDIT-WRITE-FLOW.md`](AUDIT-WRITE-FLOW.md).

Аудит разбирал путь от `/write/new/` до появления работы в каталоге без
опоры на `docs/` и нашёл три вещи, которые продакшн не выдерживали:
модерация выдавалась работе один раз и дальше не значила ничего (вторая
глава сериала и правка одобренного текста уходили читателю сами), черновик
читался по прямой ссылке кем угодно, а труд автора терялся при первой же
ошибке формы. Все три закрыты — правила BR-76…BR-89, решения DEC-66…DEC-77.
...
Оптимизация кодовой базы закончена раньше — все десять шагов и их разбор в
[`PLAN-OPTIMIZATION.md`](PLAN-OPTIMIZATION.md); отдельного приоритета у
плана нет, правила ниже уже учитывают его итог.
...
старый адрес отвечает 404. Впереди — то, что делается один раз при
деплое: зарегистрировать Telegram-бота и указать ему прод-домен
(`README.md`, раздел «Вход»).
