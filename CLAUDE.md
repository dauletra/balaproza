...
Тесты в `core/tests/` — **12 файлов, 556 тестов** (шаг 2 плана: было 25 и 1 207).
Приём везде один: класс на вопрос, один запрос в теле теста, `subTest` там,
где цикл по данным. Точечные данные приходят из `factories.py`, корпус
остаётся там, где нужен объём, — смоук и бюджет запросов.

- `test_smoke.py` — каркас портала: все маршруты в guest/auth, DEBUG-only
  design URLs, вход, контекст-процессоры, мост `messages` → тост
- `test_home.py`, `test_catalog.py`, `test_story.py`, `test_write.py` —
  главная, каталог/поиск/жанры/теги/жинақтар, страница произведения,
  авторский кабинет
- `test_profile.py` — три раздела вошедшего: профиль (свой и чужой),
  библиотека, уведомления. Правило раздела — **кто зритель** (BR-73)
- `test_contests.py` — конкурс, его страница, подача и свои заявки:
  фаза из трёх дат (DEC-45), форма ничего не отклоняет (BR-24)
- `test_lint.py` — статический лint шаблонов (многострочные `{# … #}` и
  `{% … %}` Django молча выводит текстом; Alpine вне `x-data`; утечка
  подписи в иконку) плюс регрессии каркаса: рейл только на `xl`, колонка
  контента 860px, ряды по 5 карточек, деньги через `spaced`
- `factories.py` — данные для одного теста вместо литералов корпуса
...
- `test_models.py` — модели и фильтры: публичное и приватное имя автора,
  путь тега, нормализация блок-листа, `compact_count` / `page_range`;
  справочник жанров из миграции совпадает с корпусом
...
- `test_query_budget.py` — бюджет запросов на страницу: главная, каталог, жанр,
  поиск, тег, произведение, глава, жинақтар и одна подборка + страницы
  вошедшего (профиль свой и чужой, кабинет, работа в кабинете, редактор
  главы, библиотека, уведомления, заявки, форма подачи, правка профиля).
  Ловит N+1, который зелёные тесты про содержимое не видят: свой профиль
  стоил 59 запросов, страница подборки — 76, витрина тега — 27, и суита
  при этом была полностью зелёной. **Заводить бюджет новой странице сразу**
