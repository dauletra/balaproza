...
10. админка как инструмент модерации — **сделано**;
11. удаление стаба.
...
`core/stub_data.py` больше никем не читается, кроме `seed_demo`: он остался источником демо-корпуса и уйдёт, когда литералы переедут в саму команду. Правило прежнее: **этап заканчивается удалением заменённого**, а не сосуществованием двух источников.
...
│   │   ├── site.py               # ссылки «Авторлар мектебі» (DEC-22)
...
│   │                             # Единственный модуль, читающий stub_data напрямую:
│   │                             # он конвертер между мирами, через фасад читал бы
│   │                             # то, что сам записал. Жанры не трогает — миграция
...
│   ├── stub_data.py              # источник демо-корпуса ДЛЯ СИДА (страницы его не читают):
│   │                             # Genre, Tag, Author, Story, Chapter,
...
│   │                             # outcome ХРАНИТСЯ — акт модератора, из
│   │                             # Story.status не выводится — BR-72b),
...
│   └── tests/                    # 1073 теста в 21 файле (см. ниже)
├── templates/
...
```
uv run python manage.py test core       # все 1073 теста
...
- `test_query_budget.py` — бюджет запросов на страницу: главная, каталог, жанр,
  поиск, произведение, глава, жинақтар. Ловит N+1, который зелёные тесты про
  содержимое не видят
- `test_seed.py` — `seed_demo`: идемпотентность (повтор возвращает изменённое
  к эталону) и совпадение засеянного корпуса со стабом — вплоть до бакета
  объёма и суммы реакций, от которых зависит выдача каталога
